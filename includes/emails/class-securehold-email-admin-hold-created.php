<?php
/**
 * SecureHold Email — Admin: Hold Created
 *
 * Sent to the site admin when a security deposit hold is successfully
 * authorized on Stripe.  Default: disabled (opt-in) — low signal-to-noise
...
class Securehold_Email_Admin_Hold_Created extends WC_Email {

    /** @var object|array|null  Hold payload passed to trigger(). */
    protected $hold = null;

    const LEGACY_KEY = 'admin_hold_created';

    // ── Constructor ───────────────────────────────────────────────────

    public function __construct() {
        $this->id             = 'securehold_admin_hold_created';
        $this->customer_email = false;
        $this->title          = __( 'SecureHold: Admin – Hold Created', 'securehold-security-deposit-holds' );
        $this->description    = __( 'Sent to admin when a security deposit hold is successfully created.', 'securehold-security-deposit-holds' );
        $this->template_html  = '';
        $this->template_plain = '';
        ...
        // Allow admin to override recipient via WC email settings UI.
    ...
    public function get_default_subject() {
        return __( 'New Security Hold – Order #{order_number}', 'securehold-security-deposit-holds' );
    }

    public function get_default_heading() {
        return __( 'New Security Hold Created', 'securehold-security-deposit-holds' );
    }
    ...
    public function trigger( $order_id, $hold = null ) {
        ...
        if ( $this->get_recipient() ) {
            ...
            if ( $sent && function_exists( 'securehold_log' ) ) {
                securehold_log( 'Email sent: admin_hold_created', array(
                    'order_id'  => $order_id,
    ...
    public function get_default_content() {
        ...
        return '<p>A new security deposit hold has been created.</p>
<h3>Order Details</h3>
<ul>
