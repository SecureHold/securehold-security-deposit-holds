<?php
/**
 * SecureHold Email — Admin: Hold Released
 *
 * Sent to the site admin when a security deposit hold is released without
 * capture — whether by admin manual release, Stripe webhook cancellation,
 * or auto-release cron.
 *
...
class Securehold_Email_Admin_Hold_Released extends WC_Email {

    /** @var object|array|null  Hold payload passed to trigger(). */
    protected $hold = null;

    const LEGACY_KEY = 'admin_hold_released';

    // ── Constructor ───────────────────────────────────────────────────

    public function __construct() {
        $this->id             = 'securehold_admin_hold_released';
        $this->customer_email = false;
        $this->title          = __( 'SecureHold: Admin – Hold Released', 'securehold-security-deposit-holds' );
        $this->description    = __( 'Sent to admin when a security deposit hold is released without capture.', 'securehold-security-deposit-holds' );
        $this->template_html  = '';
        $this->template_plain = '';
    ...
    public function get_default_subject() {
        return __( 'Hold Released – Order #{order_number}', 'securehold-security-deposit-holds' );
    }
    ...
    public function trigger( $order_id, $hold = null ) {
        ...
        if ( $this->get_recipient() ) {
            ...
            if ( $sent && function_exists( 'securehold_log' ) ) {
                securehold_log( 'Email sent: admin_hold_released', array(
                    'order_id'  => $order_id,
    ...
    public function get_default_content() {
        ...
        return '<p>A security deposit hold has been released without capture.</p>
<h3>Order Details</h3>
<ul>
